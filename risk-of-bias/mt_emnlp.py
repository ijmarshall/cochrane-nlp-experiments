...
skip_domains=riskofbias.CORE_DOMAINS[:4]

def main(uids_train, uids_test, target_domain="Blinding of outcome assessment", out_dir="results"):
    ...
    sent_clf = GridSearchCV(SGDClassifier(loss="hinge", penalty="L2", n_jobs=2), tuned_parameters, scoring='recall')
    '''
    tuned_parameters = {"alpha": np.logspace(-4, -3, 3), "class_weight": [{1: 10.0, -1: 1}]}
    
    # bcw: are we sure we want to do 'recall' here, and not (e.g.) F1?
    sent_clf = GridSearchCV(SGDClassifier(loss="hinge", penalty="L2", n_jobs=2), tuned_parameters, scoring='f1')
    ...
    tuned_parameters = {"alpha": np.logspace(-2, 2, 10)[:4]}
    clf = GridSearchCV(SGDClassifier(loss="hinge", penalty="L2", n_jobs=2), tuned_parameters, scoring='f1')
    ...
    X_test_d, y_test = filtered_data.Xy(uids_test, domain=target_domain, pmid_instance=0)
    ...
    for doc_text in X_test_d:
        ...
        #doc_domains = [doc_domain] * len(doc_sents)
        doc_domains = [target_domain] * len(doc_sents)
        ...
        sent_domain_interactions = ["-s-" + target_domain] * len(high_prob_sents)
        domain_interactions = [target_domain] * len(high_prob_sents)
    ...
    model_metrics.add_preds_test(y_preds, y_test, domain=target_domain)
...
def get_train_and_test_folds(folds_file="blinding-of-outcome-assessment_id_folds.txt"):
    test_fold_lists = defaultdict(list)
    all_uids = []
    with open(folds_file, 'r') as uids_to_folds:
        for row in uids_to_folds:
            uid, fold = row.strip().split(":")
            all_uids.append(uid)
            
            #
            test_fold_lists[int(fold)].append(uid)
          
    
    def build_train_uids_list(all_uids, test_uid_list):
        return [uid for uid in all_uids if not uid in test_uid_list]

    train_fold_lists = [build_train_uids_list(all_uids, test_fold_lists[i]) for i in range(5)]
    test_fold_lists  = [test_fold_lists[i] for i in range(5)]
    return zip(train_fold_lists, test_fold_lists)
